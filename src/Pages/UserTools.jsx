...
export const UserTools = () => {
  ...
  const { user } = useAuth();
  const userHistory = user ? userToolsLoaded.history.filter((transaction) => transaction.userId === user.id) : []
  const userItems = user ? userToolsLoaded.items.filter((item) => item.userId === user.id && item.status === "unavailable") : []
        ...
        <table>
          ...
          <tbody>
            {userItems.map((userItem) => {
              ...
              return (
                <tr key={userItem.id}>
                  <td>{sortByDateHistory.find((item) => item.itemId === userItem.id).timeStamp}</td>
                  <td>{userItem.id}</td>
                  <td>{userItem.name}</td>
